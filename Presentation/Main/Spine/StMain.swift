...
struct StMain{
    ...
    var body: some ReducerOf<Self> {
        ...
        Reduce {state, action in
            ...
            case .testGoToReport:
                #if DEBUG
                state.destination = .stReport(.init(report: testReportSt))
                #endif
                return .none
...
//
//#Preview {
//    StMainView(store: Store(initialState: StMain.State(who: "010111111111", dashboardTime: 0), reducer: {
//        StMain()
//    }), mode: .ready).environmentObject(BluetoothManager())
//}
