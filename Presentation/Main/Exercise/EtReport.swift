...
#if DEBUG
                        testButton(action: {
                            store.send(.setETRawData(store.setIndex ?? 0, testRawDatas))
                            store.send(.setLastSetReport(testReportETSet))
                            store.send(.setETParam(testETParams.first(where: { $0.exid == store.report.exerciseId })!))
                        })
#endif
                        Spacer()
                    }
...
#Preview {
    EtReportView(store: Store(initialState: EtReport.State(
        report: testReportET,
        lastSetReport: testReportETSet,
        rawDatas: testRawDatas,
        setIndex: 0,
        isFromLog: true
    ), reducer: {
        EtReport()
    }))
    .environmentObject(BluetoothManager.init())
//    ScrollView{
//        ETSummaryView(report: testReportET, lastReport: lastTestReportET, etParam: testETParam)
//    }
}
