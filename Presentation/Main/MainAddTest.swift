...
//                    Text("최근 측정 \(recentTime != nil ? Double(recentTime!).unixTimeToDateStr() : "-")")
//                        .font(.r_14())
//                        .foregroundStyle(.darkGraySet) //TODO (나중에)
                }
...
#Preview {
    MainListSelectPartView(store: Store(initialState: MainListSelectPart.State(), reducer: {
        MainListSelectPart()
    }))
}
