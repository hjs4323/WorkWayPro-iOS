...
struct AttachMain {
    ...
    var body: some ReducerOf<Self>{
        ...
        Reduce{ state, action in
            switch action {
            case .toggleEndBottomSheetShown: // TODO setValue 고려....? 서버요청도 중복 불가 처리
...
#Preview {
    NavigationStack {
        AttachMainView(store: Store(initialState: AttachMain.State.init(exercise: testExercises[1]), reducer: {
            AttachMain()
        }))
