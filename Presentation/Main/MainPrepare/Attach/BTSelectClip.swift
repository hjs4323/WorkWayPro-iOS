...
//                self.muscleName = Array(1..<9).map({ "\($0)번 근육" })
            }
            self.order = order
        }
    }
            ...
            WithPerceptionTracking {
                VStack(spacing: 0){
                    VStack(alignment: .leading){
                        Spacer().frame(height: geometry.size.height * 80 / 858) //TODO geometry 비율로 변경 -> 모든 클립 부착 페이지 레이아웃 통일
                        ...
                        HStack{
                            testButton(action: {
                                bluetoothManager.tempClip = testClip1
                            })
                            Spacer()
                            testButton(action: {
                                bluetoothManager.setClips(clips: testClips)
                                store.send(.delegate(.startBt(testMuscleName)))
