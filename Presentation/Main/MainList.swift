    ...
    var body: some ReducerOf<Self>{
        ...
        Reduce{ state, action in
            ...
            case let .destination(.presented(.test(.delegate(delegateAction)))):
                ...
                case let .goToMainList(report, reportType):   //TODO bt 수정 goTomainList 파라미터 바꾸기
            ...
            HStack(spacing: 0){
                ...
                VStack(alignment: .leading, spacing: 10){
                    if reportType.testType == .BRIEF {
                        ...
                        Text(name)
                            .font(.m_16())
//                        if !reports.filter({ $0 != nil }).isEmpty{ // test
                            Button(action: {
...
#Preview {
    MainListView(store: Store(initialState: MainList.State(who: "", startTime: Int(Date().timeIntervalSince1970), stReports: [nil, nil], ftReports: [nil, nil], etReports: [], btReports: []), reducer: {
        MainList()
    }))
}
