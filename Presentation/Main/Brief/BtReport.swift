                    ...
                    VStack(spacing: 0){
                        ...
                        if let btParam = store.btParam, let reportsNotToday = store.reportsNotToday {
                            ...
                            HStack{
                                ...
                                #if DEBUG
                                testButton(action: {
                                    store.send(.setBTParam(testBTParam))
                                })
                                #endif
                                Spacer()
                            }
...
//#Preview {
//    BtReportView(store: Store(initialState: BtReport.State(setIndex: 0, reports: [testReportBT], report: testReportBT, isFromLog: true), reducer: {
//        BtReport()
//    }))
