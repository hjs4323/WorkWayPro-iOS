...
//                let rawDatas = rawDatasAll.map({$0.takeLast(600)})
...
//              var labels: [String] = store.selectedReport.muscleName.map{$0.contains("번 근육") ? $0}
...
#if DEBUG
            testButton(action: {
                store.send(.testGoToWeightCount)
            })
#endif
...
#Preview {
    BtMainView(store: Store(initialState: BtMain.State(who: "", dashboardTime: 0, selectedBTName: testReportBT.name, setIndex: 0, todayReports: [testReportBT], movePrevAvailable: true), reducer: {
        BtMain()
    }))
