        ...
        VStack(alignment: .leading ,spacing: 20){
            ...
            HStack{
                if let crBorder = ftParam.crBorder[report.muscles[index].id], let standard = crBorder.first, let maxScore = crBorder.last {
                    //                        CrRateBar(muscleName: report.muscles[index].fullName(), score: report.crRates[index], lastScore: lReport?.crRates[index], maxScore: maxScore, standard: standard)
                    SingleBar(
                        title: "수축 이완비",
                        unit: "%",
...
#Preview {
    FtReportView(store: Store(initialState: FtReport.State(setIndex: 0, reports: [testReportFt, testReportFt], rawDatas: testRawDatas, ftParam: testFTParam, isFromLog: false), reducer: {
        FtReport()
    }))
}
