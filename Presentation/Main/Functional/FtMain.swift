...
struct FtMain{
    ...
    var body: some ReducerOf<Self> {
        ...
        Reduce { state, action in
            ...
            case .testGoToReport:
                #if DEBUG
                state.destination = .ftReport(.init(setIndex: 0, reports: [testReportFt], rawDatas: testRawDatas, isFromLog: false))
                #endif
                return .none
...
//#Preview {
//    FtMainView(store: Store(initialState: FtFeature.State(), reducer: {
//        FtFeature()
//    }), mode: .done).environmentObject(BluetoothManager())
//}
