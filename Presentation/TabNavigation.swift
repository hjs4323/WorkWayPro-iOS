...
struct TabNavigationView: View {
                ...
                VStack(spacing: 0){
                    ...
                    if noUpdate.contains(forceUpdateViewModel.isUpdateNeeded)  {
                        ...
                        var isBottomBar: Bool {
                            ...
                            switch store.screen{
                            case .measurement:
                                if store.measurement!.path.count > 0 { // TODO 수정 필요(나중에)
                                    return false
                                }
                            case .log:
                                return true
    ...
    //                            Button(action: {
    //                                store.send(.changeTab(.setting))
    //                            }, label: {
    //                                VStack(spacing: 5){
    //                                    Image(systemName: "person.fill")
    //                                        .foregroundColor(store.screen == .setting ? .workwayBlue : .mediumGray)
    //                                        .frame(width: geometry.size.width / tabSize, height: 24)
    //                                        .font(.system(size: 20))
    //                                    Text("설정")
    //                                        .foregroundColor(store.screen == .setting ? .white : .mediumGray)
    //                                }
    //                            })
                            }
                            .frame(height: 56)
