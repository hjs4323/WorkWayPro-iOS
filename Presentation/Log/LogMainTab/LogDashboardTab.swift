...
//            print("overallView")
//            cardCount = 0
//            if let report = store.selectedDashboard?.reportSTs, !report.isEmpty { cardCount += 1 }
//            if let report = store.selectedDashboard?.reportFTs, !report.isEmpty { cardCount += 1 }
//            if (store.selectedDashboard?.dashboardET) != nil { cardCount += 1 }
//            store.send(.setSelectedTest)
//        }
    }
...
#Preview {
    LogDashboardTabView(store: Store(initialState: LogDashboardTab.State.init(
        who: "123123123",
