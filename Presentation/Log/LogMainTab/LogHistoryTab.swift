...
//            case .goToDetailBT:
//                state.destination = .detailBT(.init(who: state.who, dashboards: state.dashboards, selectedList: state.selectedList, btParam: state.btParam))
//                return .none
...
//                        let btReportsArr: [[ReportBTDTO]] = Array(store.selectedList.compactMap({ $0.reportBTs }).filter({ !$0.isEmpty }).prefix(6).reversed())
//                        if !btReportsArr.flatMap({ $0 }).isEmpty{
//                            VStack(spacing: 0){
//                                Spacer().frame(height: 30)
//                                BTSummaryCardView(
//                                    geometry: geometry,
//                                    btReportsArr: btReportsArr,
//                                    btParam: store.btParam,
//                                    goToDetailBT: {
//                                        store.send(.goToDetailBT)
//                                    }
//                                )
//                                Spacer().frame(height: 30)
//                            }
//                            .background(.white)
//                            .onAppear {
//                                print("btReportsArr = \(btReportsArr)")
//                            }
//                        }
...
//                .navigationDestination(item: $store.scope(state: \.destination?.detailBT, action: \.destination.detailBT)) { store in
//                    LogHistoryDetailBTView(store: store)
//                }
            }
        }
    }
...
//    struct BTSummaryCardView: View {
//        var geometry: GeometryProxy
//        var btReportsArr: [[ReportBTDTO]]
//        var btParam: BTParam
//        let goToDetailBT: () -> ()
//        @State var index: Int = 0
//        
//        var body: some View {
////            let timeIntervals = btReportsArr.compactMap({$0.first != nil ? Double($0.first!.dashboardTime) : nil})
//            let timeIntervals = btReportsArr.compactMap({ $0.first?.dashboardTime }).map({ Double($0) })
//            let dateTimes: [String] = timeIntervals.map({$0.unixTimeToDateStr("( MM.dd hh:mm )")})
//            let dates: [String] = timeIntervals.unixTimeToDateStrForGraph()
//            let times: [String] = timeIntervals.map({$0.unixTimeToDateStr("hh:mm")})
//            
...
//                            Text(String(Int(btReportsArr.flatMap({$0}).map({$0.max.first ?? 0}).max() ?? 0)))
//                                .font(.s_22())
//                                .foregroundStyle(.lightBlack)
...
//                                .foregroundStyle(.darkGraySet)
//                            Spacer().frame(width: 20)
//                            Text(String(Int(btReportsArr.flatMap({$0}).map({$0.max.last ?? 0}).max() ?? 0)))
//                                .font(.s_22())
//                                .foregroundStyle(.lightBlack)
//                            Spacer().frame(width: 7)
//                            Text("%")
...
//                                .foregroundStyle(.darkGraySet)
//                            Spacer().frame(width: 20)
//                            Text(String(Int(btReportsArr.flatMap({$0}).map({$0.mean.first ?? 0}).max() ?? 0)))
//                                .font(.s_22())
//                                .foregroundStyle(.lightBlack)
//                            Spacer().frame(width: 7)
//                            Text("%")
...
//                                .foregroundStyle(.darkGraySet)
//                            Spacer().frame(width: 20)
//                            Text(String(Int(btReportsArr.flatMap({$0}).map({$0.mean.last ?? 0}).max() ?? 0)))
//                                .font(.s_22())
//                                .foregroundStyle(.lightBlack)
//                            Spacer().frame(width: 7)
//                            Text("%")
...
//                        label: dates,
//                        subLabel: times,
//                        score: btReportsArr.map({ $0.map({ $0.weight }).average() }),
//                        color: .darkGraySet,
//                        select: $index,
//                        height: 170,
//                        width: geometry.size.width - 80
...
#Preview {
    LogHistoryTabView(store: Store(initialState: LogHistoryTab.State.init(
        who: "123123123",
