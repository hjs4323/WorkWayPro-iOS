...
class BluetoothManager: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    //            self.hubLowBatteryAlert = 3.3 < battery && battery < 3.4 && !hub.battAlerted //TODO hublowbattery 조건 -> view에서 이 조건으로 바로 팝업 띄우기
//                self.hub.died = battery < 3.3
            }
        }
        
//        state.clipLowBattery = workoutRepository.clips.filter({ 3.3 < $0.battery! && $0.battery! < 3.4 && !$0.battAlerted})
//        state.clipLowBatteryAlert = !state.clipLowBattery.isEmpty
//        state.clipDead = workoutRepository.clips.filter({ $0.battery! < 3.3 })
//        state.clipDiedAlert = !state.clipDead.isEmpty
    }
...
//                // 사용자에게 Bluetooth를 켜도록 요청하는 UI를 표시
//                let alertController = UIAlertController(title: "블루투스 연결", message: "운동을 시작하려면 디바이스의 블루투스를 켜주세요", preferredStyle: .alert)
//                alertController.addAction(UIAlertAction(title: "설정", style: .default, handler: { action in
//                    // 사용자가 "OK"를 누르면 iOS 설정 앱의 Bluetooth 섹션으로 이동
...
//        guard let lengthInt else {
//            print("length error")
//            return nil
//        }
//        let checksum = String(lengthInt + dataInt)
//        if checksum.dropFirst(checksum.count-4) != checkSum {
...
//        }
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
        ...
        if let message = data?.hexEncodedString(.upperCase) {
            ...
            switch command {
            case "E0":
                if payload.takeLast(2) != "00" { print("에러") } //TODO 토스트 띄우기
...
//            case "A4":
//                let clips = WorkoutRepository.shared.clips
//                var strangeConnection: [Clip] = []
//                let connectionStatuses = payload.components(withMaxLength: 14)
//                connectionStatuses.forEach { macStatus in
//                    let mac = String(macStatus.dropLast(2))
//                    let status = macStatus.takeLast(2)
//                    if status != "01" && clips.map({ $0.macAddress }).contains(mac) {
//                        if let strangeClip = clips.first(where: { $0.macAddress == mac }) {
//                            strangeConnection.append(strangeClip)
//                        }
//                    }
//                }
//                if !strangeConnection.isEmpty {
//                    alertDelegate?.clipDisconnection(strangeConnection)
//                }
            case "A5":
...
//            if tappedRawArrCounts.count > 1 && averageArray.filter({ $0.count != 0 }).last?.count == tappedRawArrCounts[1] - tappedRawArrCounts[0], let currentExercise {
//                Task {
//                    await dataLogicTrigger(exercise: currentExercise, weight: self.weight, retry: 0) {} onFailure: {}
//                }
//            }
        }
    }
    ...
    func disconnect() async {
        ...
        if let connectedPeripheral {
            centralManager.cancelPeripheralConnection(connectedPeripheral) //TODO 테스트
        }
    }
}
